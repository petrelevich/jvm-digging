...
public class FunctionalEndpointsConfig {
    private static final String PERSON_ENDPOINT = "/func/person";
    ...
    public RouterFunction<ServerResponse> composedRoutes(PersonRepository repository) {
        return route()
                ...
                .GET(PERSON_ENDPOINT, queryParam("name", param -> !param.isEmpty()), request -> request.queryParam(
                                "name")
                ...
                .GET(PERSON_ENDPOINT, queryParam("age", param -> !param.isEmpty()), request -> ok().contentType(
                                MediaType.APPLICATION_JSON)
                        .body(
                                repository.findAllByAge(request.queryParam("age")
                                        .map(Integer::parseInt)
                                        .orElseThrow(IllegalArgumentException::new)),
                                Person.class))
                // Обратите внимание на использование хэндлера
                .GET(PERSON_ENDPOINT, accept(APPLICATION_JSON), new PersonHandler(repository)::list)
                // Обратите внимание на использование pathVariable
                .GET("/func/person/{id}", accept(APPLICATION_JSON), request -> repository
